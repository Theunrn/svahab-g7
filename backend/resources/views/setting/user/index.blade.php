          ...
          <div class="text-right">
            @can('User create')
              <a href="{{ route('admin.users.create') }}"
                class="bg-blue-500 text-white font-bold px-5 py-2 rounded focus:outline-none shadow hover:bg-blue-600 transition-colors duration-300">New User</a>
            @endcan
          </div>
        </div>
        ...
        <div class="bg-white shadow-md rounded my-6">
          <table class="min-w-full border-collapse">
            ...
            <tbody>
              @can('User access')
                @foreach($users as $user)
                  <tr v-if="filterByRole('{{ $user->roles->pluck('id') }}')">
                    <td class="py-2 px-6 border-b border-gray-300">
                      <div class="flex items-center">
                        <div class="h-10 w-10">
                          <img src="{{ $user->profile ? asset('images/' . $user->profile) : asset('images/default-profile.jpg') }}"
    ...
    export default {
      setup() {
        ...
        const allRoles = computed(() => {
          ...
          @foreach($users as $user)
            @foreach($user->roles as $role)
              roles.add({ id: {{ $role->id }}, name: '{{ $role->name }}' });
            @endforeach
          @endforeach
          return Array.from(roles);
        });

        const filterByRole = (userRoles) => {
          if (!selectedRole.value) {
            return true; // If no role is selected, show all users
          }
          return userRoles.includes(selectedRole.value); // Filter users by selected role
        };
