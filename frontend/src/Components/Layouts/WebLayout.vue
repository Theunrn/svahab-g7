<template>
      ...
      <div class="text-center mx-auto">
        ...
        <div class="relative w-full">
          <input
            ...
            @input="searchFields"
            type="text"
            placeholder="Search Find Field..."
            class="w-full px-4 py-2 rounded-md text-black"
          />
          <button
            @click="searchFields"
            type="button"
            class="absolute right-0 top-0 bg-red-500 text-white px-4 py-2 rounded-md"
          >
            Search
          </button>
        </div>
  ...
  <FootballFields :newFields="matchFields" />
  <GoogleMap />
</template>
...
const filterFields = () => {
  matchFields.value = fields.value.filter((field) => {
    return field.province.toLowerCase() === selectedProvince.value.toLowerCase();
  });
}
watch(selectedProvince, filterFields)
onMounted(() => {
  fetchFields()
